...
public class DeclarationPostController {
    ...
    public ResponseEntity<?> deleteDPost(@PathVariable Long id) {
        return ResponseEntity.ok( declarationPostService.deleteDPost(id));
    }

    //신고 게시글 조회 (관리자만)
    @GetMapping("/getList")
    public ResponseEntity<?> getDPostList(@RequestParam int page, @RequestParam int size) {
        return  ResponseEntity.ok(declarationPostService.getDPostAdmin(PageRequest.of(page,size)));
    }
    //신고 게시글 조회 (본인거)
    @GetMapping("/getList/{nickname}")
    @Operation(summary = "신고 게시글 조회", description = "신고 게시글을 조회합니다. 관리자는 모든 게시물을 조회할 수 있으며, 글을 쓴 작성자는 본인의 게시물만을 확인할 수 있습니다.")
    public ResponseEntity<?> listDPost(@PathVariable String nickname, @RequestParam int page, @RequestParam int size) {
        return ResponseEntity.ok(declarationPostService.getDPost(nickname, PageRequest.of(page,size)));
    }
    ...
    public ResponseEntity<?> getAPostById(@PathVariable Long postId) {
        return ResponseEntity.ok(declarationPostService.getPostDetail(postId));
    }
}
